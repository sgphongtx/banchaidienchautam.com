...
class Csrf
{
    ...
    function __construct($use_token = true, $token_post = false, $token_get = false)
    {
        ...
        if ($token_post && !$this->__validate_post()){
            $this->remove_token();
            echo "<script>location.href = \"/quantri/\"</script>";
        }
    ...
    private function __create_csrf_token()
    {
        ...
        if (!isset($_SESSION["user"][$this->_csrf_token_name]))
        {
            // Tạo token
            $token = md5($_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'].rand(0,1000));

            // Lưu token trong 1h
            $_SESSION["user"][$this->_csrf_token_name] = $token;
        }
        else{
            $token = $_SESSION["user"][$this->_csrf_token_name];
            $_SESSION["user"][$this->_csrf_token_name] = $token;
        }
    ...
    private function __validate_post()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            if (!isset($_POST[$this->_csrf_token_name]) || !isset($_SESSION["user"][$this->_csrf_token_name])){
                return false;
            }
            // Nếu token không phù hợp
            else if ($_POST[$this->_csrf_token_name] != $_SESSION["user"][$this->_csrf_token_name]){
                return false;
            }
        }
        return true;
    ...
    private function __validate_get()
    {
        ...
        {
            // Kiểm tra có tồn tại token không
            if (!isset($_GET[$this->_csrf_token_name]) || !isset($_SESSION["user"][$this->_csrf_token_name])){
                return false;
            }
            // Nếu token không phù hợp
            else if ($_GET[$this->_csrf_token_name] != $_SESSION["user"][$this->_csrf_token_name]){
                return false;
            }
        }
        return true;
    ...
    function remove_cookie(){
        setcookie(md5("un"), "",time()-1);
        setcookie(md5("pw"), "",time()-1);
    }
}