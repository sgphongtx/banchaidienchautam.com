...
<?php        
    switch ($_GET['action']){
        case 'del' :
            ...
            if($r['idshop']==$idshop){
                $resultParent = mysqli_query($conn,"select id from tbl_video where parent='".$id."'");
                if (mysqli_num_rows($resultParent) <= 0){
                    @$result = mysqli_query($conn,"delete from tbl_video where id='".$id."'");
                    if ($result){
...
<div class="content bg-gray-lighter">
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">Video <small>Danh sách Video</small></h1>
        </div>
                    ...
                    <table class="table table-bordered table-hover btl-list-bbli bg-white">
                        <thead>
                            <tr>
                                ...
                                <td style="width: 350px;" class="text-center">Video</td>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="embed-responsive embed-responsive-4by3" id="video_return">
                                        <iframe class="embed-responsive-item" src="<?php echo get_video_youtobe($row['link']); ?>?autoplay=0"></iframe>
                                    </div>
                                </td>
                                <td class="text-left">
                                    <?=$row['name']?>
                                ...
                                <td class="text-center">
                                    <div class="check tool <?php echo $row['status']==1?'active':'' ?>" data-action='toggleStatus' data-field='status' data-table='tbl_video' title="Ẩn hiện" value="<?=$row['id']?>"></div>
                                </td>
                                <td class="text-right">
