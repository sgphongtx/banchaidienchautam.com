...
if (isset($_POST['btnSave'])){
   ...
   $parent       = $_POST['ddCat'];
   $parent       = isset($_POST['name']) ? trim($_POST['ddCat']) : $lang;
   ...
   $link         = isset($_POST['link']) ? trim($_POST['link']) : '';
	...
	$level = get_one_field("tbl_item_category","id='$parent'",$lang,"level");
	$level = $level+1;
	...
	if ($errMsg=='') {
		...
		if (!empty($_POST['id'])){	
			$oldid = $_POST['id'];
			$image_ = $image;
			$sql = "UPDATE tbl_item_category SET idshop = '$idshop', name = '$name', parent = '$parent', subject = '$subject', subject_temp = '$subject_temp', detail_short = '$detail_short', image='$image_', link='$link', sort = '$sort', title = '$title', description = '$description', keyword = '$keyword', lang = '$lang', last_modified = now() WHERE id = '$oldid'";
		}else{
      	$image_ = $image;
			$sql = "INSERT INTO tbl_item_category (idshop, name, cate, parent, subject, subject_temp, detail_short, image, link, sort, title, description, keyword, status, lang, date_added, last_modified) VALUES ('$idshop', '$name', '1', '$parent', '$subject', '$subject_temp', '$detail_short', '$image_', '$link', '$sort', '$title', '$description', '$keyword', '1', '$lang', now(), now())"; }
   ...
   <div class="row items-push">
      ...
      <div class="col-sm-5 text-right hidden-xs">
         <ol class="breadcrumb push-10-t">
            ...
            <li><a href="<?=url_direct('get')?>">Danh mục</a>
            </li>
            <li>Thêm mới</li>
         </ol>
      </div>
                  ...
                  <div class="panel panel-default panel-light">
                     <div class="panel-body">
                        ...
                        <div class="form-group">
                           ...
                           <select name="ddCat" class="form-control" id="menu_parent_id_<?php echo $oldid ?>">
                        ...
                        <div class="form-group">
                           ...
                           <textarea name="txtDetailShort" class="form-control tinymce-editor" id="txtDetailShort"><?=$detail_short;?></textarea>
                        </div>
                     </div>
                  </div>
               </div>
                  ...
                  <div class="panel panel-default panel-light">
                     <div class="panel-body">
                        ...
                        <div class="form-group">
                           ...
                           <textarea name="description" class="form-control countW" id="seo_description" maxlength="160">
                              <?=$description?>
                           </textarea>
                        </div>
                        <div class="form-group">
   ...
   $('#seo_description').closest('.form-group').find('.elcount').html(value_ol2);
	...
	$('a[data-action="default-alias"]').on('click', function(){
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: {'value': value, 'id': '<?php echo $oldid ?>', 'table': 'tbl_item_category', 'cmd': 'DEF_ALIAS'},
		})
		.done(function(data) {
			$('input[name="subject"]').val(data.subject);
			$('input[name="subject_tmp"]').val(data.subject_temp);
