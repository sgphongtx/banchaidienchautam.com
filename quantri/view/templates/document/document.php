...
<?php
    switch ($_GET['action']){
        case 'del' :
            $id = $_GET['id'];
            $r = getRecord("tbl_document","id=".$id);
            if($r['idshop']==$idshop){
                @$result = mysqli_query($conn,"delete from tbl_document where id='".$id."'");
                if ($result){
...
<div class="content bg-gray-lighter">
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">Tài liệu <small>Danh sách Tài liệu</small></h1>
        </div>
    ...
    <div class="block">
        ...
        <div class="block-content">
            ...
            <?php
                ...
                $totalRows=countRecord("tbl_document",$where);
            ?>
                    ...
                    <table class="table table-bordered table-hover btl-list-bbli bg-white">
                        <thead>
                            <tr>
                                ...
                                <td style="width: 90px;" class="text-right">Công cụ</td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                                ...
                                $sql="select * from tbl_document where $where $sortby $direction limit ".($startRow).",".$pageSize;
                            ...
                            <tr>
                                ...
                                <td class="text-left">
                                    Tên : <?=$row['name']?><br />
                                    Link : <a href="<?php echo __PATH_UPLOAD__.$row['url']?>"> <?php echo __PATH_UPLOAD__.$row['url']?> </a><br />
                                </td>
                                <td class="text-center">
                                    <div class="check tool <?php echo $row['status']==1?'active':'' ?>" data-action='toggleStatus' data-field='status' data-table='tbl_document' title="Ẩn hiện" value="<?=$row['id']?>"></div>
                                </td>
                                <td class="text-right">
