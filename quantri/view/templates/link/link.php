...
<?php        
    switch ($_GET['action']){
        case 'del' :
            $id = $_GET['id'];
            $r = getRecord("tbl_link","id=".$id);
            if($r['idshop']==$idshop){
                $resultParent = mysqli_query($conn,"select id from tbl_link where parent='".$id."'");
                if (mysqli_num_rows($resultParent) <= 0){
                    @$result = mysqli_query($conn,"delete from tbl_link where id='".$id."'");
                    if ($result){
...
<div class="content bg-gray-lighter">
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">Liên kết <small>Danh sách liên kết website - mạng xã hội</small></h1>
        </div>
    ...
    <div class="block">
        ...
        <div class="block-content">
            ...
            <?php
                ...
                $totalRows=countRecord("tbl_link",$where);
            ?>
                    ...
                    <table class="table table-bordered table-hover btl-list-bbli bg-white">
                        <thead>
                            <tr>
                                ...
                                <td class="text-center">Loại liên kết</td>
                        ...
                        <tbody>
                            <?php
                                ...
                                $sql="select * from tbl_link where   $where $sortby $direction limit ".($startRow).",".$pageSize;
                            ...
                            <tr>
                                ...
                                <td class="text-center">                                    
                                    <?php 
                                        if($row['cate']==0) echo '<span class="label label-danger">Liên kết website</span>'; 
                                        elseif($row['cate']) echo '<span class="label label-info">Liên kết mạng xã hội</span>'; 
                                    ?>
                                </td>
                                <td class="text-center">
                                    <div class="check tool <?php echo $row['status']==1?'active':'' ?>" data-action='toggleStatus' data-field='status' data-table='tbl_link' title="Ẩn hiện" value="<?=$row['id']?>"></div>
                                </td>
                                <td class="text-right">
