...
<?php        
    ...
    case 'del' :
        ...
        if($r['idshop']==$idshop){
            $resultParent = mysqli_query($conn,"select id from tbl_support where parent='".$id."'");
            if (mysqli_num_rows($resultParent) <= 0){
                @$result = mysqli_query($conn,"delete from tbl_support where id='".$id."'");
                if ($result){
                    ...
                    <table class="table table-bordered table-hover btl-list-bbli bg-white">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-left">
                                    Tên         : <?=$row['name']?><br />
                                    Zalo       : <?=$row['nickyahoo']?><br />
                                    Skype       : <?=$row['nickskype']?><br />
                                    Điện thoại  : <?=$row['dienthoai']?>
                                </td>
                                <td class="text-center">                                
                                    <div class="check tool <?php echo $row['status']==1?'active':'' ?>" data-action='toggleStatus' data-field='status' data-table='tbl_support' title="Ẩn hiện" value="<?=$row['id']?>"></div>
                                </td>
                                <td class="text-right">
