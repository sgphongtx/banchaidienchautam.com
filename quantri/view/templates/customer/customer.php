<?php        
    switch ($_GET['action']){
        case 'del' :
            $id = $_GET['id'];
            $r = getRecord("tbl_customer_shop","id=".$id);
            if($r['idshop']==$idshop){
                $resultParent = mysqli_query($conn,"select id from tbl_customer_shop where parent='".$id."'");
                if (mysqli_num_rows($resultParent) <= 0){
                    @$result = mysqli_query($conn,"delete from tbl_customer_shop where id='".$id."'");
                    if ($result){
                        $errMsg1 = "Đã xóa thành công.";
                    }else $errMsg2 = "Không thể xóa dữ liệu !";
                }else{
                    $errMsg2 = "Đang có danh mục sử dụng. Bạn không thể xóa !"; 
                }
            }
            break;
    }
...
<div class="content bg-gray-lighter">
    <div class="row items-push">
        <div class="col-sm-7">
            <h1 class="page-heading">Thành viên</h1>
        </div>
...
<div class="content" style="min-height: 530px;">
    <div class="block">
        <div class="block-header bg-gray-lighter">
            ...
            <button type="button" onclick="location.href='view/templates/customer/emaiLexport.php'" class="btn btn-primary btn-sm pull-right"><i class="fa fa-file-excel-o"></i> Xuất file excel</button>
        </div>
        <div class="block-content">
            ...
            <?php                
                ...
                $where="1=1";
                $totalRows=countRecord("tbl_customer_shop",$where);
            ?>
                    ...
                    <table class="table table-bordered table-hover btl-list-bbli bg-white">
                        <thead>
                            <tr>
                                ...
                                <td class="text-left">Thông tin</td>
                        ...
                        <tbody>
                            <?php
                                ...
                                $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_customer_shop where $where $sortby $direction limit ".($startRow).",".$pageSize;
                            ...
                            <tr>
                                ...
                                <td class="text-left">
                                    Họ và tên thành viên : <?=$row['name']?> <br />
                                    Giới tính : <?php if($row['sex']==1) echo "Nam";else echo "Nữ";?> <br />
                                    Địa chỉ : <?=$row['address']?> <br />
                                    Điện thoại : <?=$row['mobile']?> <br />
                                    Email : <?=$row['email']?>
                                </td>
                                <td class="text-center">
                                    <div class="check tool <?php echo $row['status']==1?'active':'' ?>" data-action='toggleStatus' data-field='status' data-table='tbl_customer_shop' title="Ẩn hiện" value="<?=$row['id']?>"></div>
                                </td>
                                <td class="text-right">
                                    <a href="<?=url_direct('edit',$_GET['act'],'_m','&page='.$_REQUEST['pageNum'].'&id='.$row['id'])?>" title="Sửa" class="btn btn-xs btn-pencil btn-primary"> <i class="fa fa-pencil"></i> </a>
                                    <a href="<?=url_direct('del',$_GET['act'],null,'&action=del&page='.$_REQUEST['pageNum'].'&id='.$row['id'])?>" title="Xóa"  onclick="return confirm('Bạn có muốn xoá luôn không ?');"  class="btn btn-xs btn-remove btn-danger"> <i class="fa fa-trash"></i> </a>
                                </td>
                            </tr>
                            <?php } ?>
                        </tbody>
